...
room_queue_msg_threadpool = ThreadPoolExecutor(max_workers=500)
...
def push_room_msg(room_id):
    try:
        ...
        if(room_id in room_msg_queue_relations):
            room_msg_queue_relations[room_id].put(new_frams)
            for single_action in new_frams:
                single_action['add_action_queue_time'] = time.time()
        else:
            logger.error(f"room {room_id} not exist msg queue, pls check logic")
...
def start_room_msg_push_thread(room_id):
    # logger.info(f"start_room_msg_push_thread {room_id}")
    new_loop = asyncio.new_event_loop()
    try:
        t = threading.Thread(target=start_room_msg_push_loop, args=(new_loop, room_id))
        t.start()
        pass
    except Exception as e:
...
def start_room_msg_push_loop(loop, room_id):
    # asyncio.***  的操作是基于当前 loop 环境的,
    # 因此需要这行下面的设置 loop 环境, 
    asyncio.set_event_loop(loop)
    #  this is more more important !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    asyncio.set_event_loop_policy(uvloop.EventLoopPolicy())
    loop.run_until_complete(handling_room_msg_frame(room_id)) 

    
async def handling_room_msg_frame(room_id):
    if(room_id not in room_msg_queue_relations):
        logger.error(f"room:{room_id} has no msg queue")
        raise
    room_msg_queue = room_msg_queue_relations[room_id]
    logger.debug(f"handing room {room_id} msg process start")
    
    while  True:
        new_frame = room_msg_queue.get(block=True, timeout=None)
        for single_action in new_frame:
                single_action['get_action_queue_time'] = time.time()

        logger.debug(f"new frame {new_frame} in room:{room_id}")
        ...
        if("player_1" in room_info) :
            ...
            sync_tasks.append(asyncio.ensure_future(send_new_frame(player_1_id, new_frame)))

        if("player_2" in room_info) :
            room_player_count+=1;
            player_2_id = room_info["player_2"]
            logger.debug("player 2: " + player_2_id)
            sync_tasks.append(asyncio.ensure_future(send_new_frame(player_2_id, new_frame)))
        ...
        if(len(sync_tasks) > 0):
            ...
            try:
                ...
                logger.error(f"send room frame exception, room:{room_id}", exc_info=True)
            finally:
                pass
...
def create_room(action_json):
    ...
    room_msg_queue_relations[room_id] = queue.Queue()

    start_room_msg_push_thread(room_id)
...
def make_move(action_json):
    ...
    action_json['add_room_queue_time'] = time.time()

    # if(room_id == '3dd-d-'):
    if(len(rooms_moves[room_id]['all']) > 10000):
        return
...
async def send_new_frame(player_id, new_frame):
    if(player_id in player_socket_relations) :
        logger.debug("send msg to player: " + player_id)
        player_websocket = player_socket_relations[player_id] 
        try:
            for single_fram in new_frame:
                single_fram['start_send_msg_time'] = time.time()
            await player_websocket.send(sync_frame_event(new_frame))
            # for single_action in new_frame:
            #     single_action['finish_send_msg_time'] = time.time()
            #     logger.debug(f"send action finish {single_action}")
        except Exception as e:
        ...
        logger.error(f"player not has socket {player_id}")
